...
//    let locationManage = LocationManager.shared
...
extension UberMapViewRepresentable {
    ...
    class MapCoordinator: NSObject, MKMapViewDelegate {
        ...
        func configurePolyline(withDestinationCoordinate coordinate: CLLocationCoordinate2D) {
            ...
            parent.locationViewModel.getDestinatoinRoute(from: userLocationCoordinate,
                                                         to: coordinate) { [weak self] route in
                self?.parent.mapView.addOverlay(route.polyline)
                self?.parent.mapState = .polylineAdded
                if let rect = self?.parent.mapView.mapRectThatFits(route.polyline.boundingMapRect,
